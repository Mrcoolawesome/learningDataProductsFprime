...
In order to build the Test application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the Test application's code.
```
fprime-util build
```
...
To run the ground system without starting the Test app:
```
...
```
cd Test/build-artifacts/<platform>/bin/
./Test -a 127.0.0.1 -p 50000
```
